import React, { useState } from 'react';
import { useHederaContext } from '../contexts/HederaContext';
import {
  ContractCreateTransaction,
  ContractExecuteTransaction,
  ContractFunctionParameters
} from '@hashgraph/sdk';
import '../style/SmartContractInteraction.css';

function SmartContractInteraction() {
  const { client } = useHederaContext();
  const [contractId, setContractId] = useState('');
  const [message, setMessage] = useState('');
  const [retrievedMessage, setRetrievedMessage] = useState('');
  const [error, setError] = useState('');

  const contractBytecode = '';

  const handleDeployContract = async () => {
    if (!client) {
      setError('Client is not available.');
      return;
    }

    try {
      const transaction = await new ContractCreateTransaction()
        .setBytecode(contractBytecode)
        .setGas(100000)
        .freezeWith(client);

      const signTx = await transaction.sign(client.operatorKey);
      const txResponse = await signTx.execute(client);
      const receipt = await txResponse.getReceipt(client);
      setContractId(receipt.contractId.toString());
      setError('');
    } catch (error) {
      setError('Error deploying contract: ' + error.message);
      console.error('Error deploying contract:', error);
    }
  };

  const handleSetMessage = async (e) => {
    e.preventDefault();
    if (!contractId || !message || !client) {
      setError('Contract ID, message, or client is missing.');
      return;
    }

    try {
      const transaction = await new ContractExecuteTransaction()
        .setContractId(contractId)
        .setGas(100000)
        .setFunction("setMessage", new ContractFunctionParameters().addString(message))
        .freezeWith(client);

      const signTx = await transaction.sign(client.operatorKey);
      const txResponse = await signTx.execute(client);
      await txResponse.getReceipt(client);
      setMessage('');
      setError('');
      console.log('Message set successfully');
    } catch (error) {
      setError('Error setting message: ' + error.message);
      console.error('Error setting message:', error);
    }
  };

  const handleGetMessage = async () => {
    if (!contractId || !client) {
      setError('Contract ID or client is missing.');
      return;
    }

    try {
      const transaction = await new ContractExecuteTransaction()
        .setContractId(contractId)
        .setGas(100000)
        .setFunction("getMessage")
        .freezeWith(client);

      const signTx = await transaction.sign(client.operatorKey);
      const txResponse = await signTx.execute(client);
      const result = await txResponse.getRecord(client);
      const message = result.contractFunctionResult.getString(0);
      setRetrievedMessage(message);
      setError('');
    } catch (error) {
      setError('Error getting message: ' + error.message);
      console.error('Error getting message:', error);
    }
  };

  return (
    <div className="smart-contract-interaction">
      <h2>Smart Contract Interaction</h2>
      {error && <p className="error-message">{error}</p>}
      <button className="deploy-contract-button" onClick={handleDeployContract}>Deploy Contract</button>
      {contractId && (
        <div className="contract-section">
          <p>Contract ID: {contractId}</p>
          <form className="message-form" onSubmit={handleSetMessage}>
            <input
              className="message-input"
              type="text"
              placeholder="Enter message"
              value={message}
              onChange={(e) => setMessage(e.target.value)}
            />
            <button className="set-message-button" type="submit">Set Message</button>
          </form>
          <button className="get-message-button" onClick={handleGetMessage}>Get Message</button>
          {retrievedMessage && <p className="retrieved-message">Retrieved Message: {retrievedMessage}</p>}
        </div>
      )}
    </div>
  );
}

export default SmartContractInteraction;